...
const TOOL_ICONS = { read_revenue: "file-text", read_users: "activity", list_insights: "zap", read_setup: "badge-check" };

const thoughtMarkup = (text, seconds, at) => `<div class="c-note">${escapeHtml(String(text).replace(/\*\*/g, "").replace(/^#+ /gm, ""))}${at ? `<span class="app-turn__when">${relTime(at)}</span>` : ""}</div>`;
...
const milestoneMarkup = (it) => `<div class="c-milestone${it.src ? " is-viewable" : ""}"${it.src ? ` role="button" tabindex="0" data-view-src="${escapeHtml(it.src)}" data-view-title="${escapeHtml(it.title)}"` : ""}>${it.url ? `<a class="c-milestone__mark" href="${it.url}" target="_blank" rel="noopener">${milestoneMark(it.icon)}</a>` : `<span class="c-milestone__mark">${milestoneMark(it.icon)}</span>`}<span class="c-milestone__body"><span class="c-milestone__title">${it.url ? `<a href="${it.url}" target="_blank" rel="noopener">${escapeHtml(it.title)}</a>` : escapeHtml(it.title)}</span>${it.text ? `<span class="c-milestone__text">${escapeHtml(it.text)}</span>` : ""}</span>${it.url ? `${icon("arrow-up-right", "c-milestone__open")}` : it.src ? `${icon("play", "c-milestone__open")}` : ""}</div>`;
// the live loop sends a tool's arguments as a JSON object (server/loop.js),
// the sim passes a pre-formatted string — draw both as `key: value` pairs,
// long values clipped so a write_file's body never floods the summary line
const ARGS_CLIP = 80;
const argValue = (v) => {
  const text = typeof v === "string" ? v : JSON.stringify(v) ?? String(v);
  return text.length > ARGS_CLIP ? `${text.slice(0, ARGS_CLIP - 1)}…` : text;
};
const formatArgs = (args) => {
  if (args == null) return "";
  if (typeof args === "string") return args;
  try {
    if (Array.isArray(args)) return args.map(argValue).join(", ");
    return Object.entries(args).map(([k, v]) => `${k}: ${argValue(v)}`).join(", ");
  } catch {
    return String(args);
  }
};
// a tool's arguments and the agent's lines carry links to what they act on
// ([label](url) in the pools); your own lines stay plain text
const toolMarkup = (t, done) => `<details class="c-tool${done ? " is-done" : ""}"><summary><span class="c-tool__icon">${icon(t.icon || TOOL_ICONS[t.name] || "file-text")}</span><span class="c-tool__call"><span class="c-tool__name">${escapeHtml(t.subagent ? `${t.subagent} · ${t.name}` : t.name)}</span><span class="c-tool__args">${richText(formatArgs(t.args))}</span></span><span class="c-tool__status">${done ? icon("check") : icon("loader-circle", "c-tool__spin")}</span></summary><pre class="c-tool__result">${escapeHtml(t.result || "")}</pre></details>`;
...
const subagentMarkup = (it) => {
  ...
  <span class="app-submsg__text">${escapeHtml(it.text || "")}</span>
</div>`;
};
...
const msgMarkup = (who, rawText, reply, files) => {
...
function threadMarkup(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (it.kind === "thought") {
      flush();
      turn += thoughtMarkup(it.text, it.seconds, it.at);
    } else if (it.kind === "tool") {
      // a run of tool calls draws as one quiet line, not a stack of cards
      const names = [it.name];
      while (items[i + 1]?.kind === "tool") names.push(items[++i].name);
      turn += `<div class="c-burst">${icon("activity")}<span class="c-burst__names">${escapeHtml(names.join(", "))}</span></div>`;
    }
      ...
      else if (it.kind === "subagent") out += subagentMarkup(it);
...
export function renderChat(root, projectId) {
  ...
  // ---------- pausing: Pause asks first, Resume is one press ----------
  ...
  async function incoming(item) {
    ...
    if (item.kind === "thought") {
      ...
      liveTurn.className = "c-turn";
      liveTurn.innerHTML = thoughtMarkup(item.text, item.seconds, item.at);
      thread.append(liveTurn);
      follow();
      if (!reduceMotion()) liveTurn.animate([{ opacity: 0, transform: "translateY(4px)" }, { opacity: 1, transform: "translateY(0)" }], { duration: 200, easing: EASE_OUT });
    } else if (item.kind === "tool") {
      // a run of live tool calls grows one quiet line instead of a card stack
      if (!liveTurn || !liveTurn.isConnected) {
        liveTurn = document.createElement("div");
        liveTurn.className = "c-turn";
        thread.append(liveTurn);
      }
      let burst = liveTurn.querySelector(".c-burst:last-child");
      if (!burst) {
        liveTurn.insertAdjacentHTML("beforeend", `<div class="c-burst">${icon("activity")}<span class="c-burst__names">${escapeHtml(item.name)}</span></div>`);
        burst = liveTurn.lastElementChild;
      } else {
        burst.querySelector(".c-burst__names").textContent = `${burst.dataset.names || item.name}, ${item.name}`;
      }
      burst.dataset.names = burst.querySelector(".c-burst__names").textContent;
      follow();
      if (!form.dataset.busy) setNow(`Running ${item.name}`);
      await wait(reduceMotion() ? 100 : 200);
      ...
      liveTurn = null;
      const card = append(subagentMarkup(item), { force: true });
      if (!reduceMotion()) card.animate([{ opacity: 0, transform: "translateY(8px)" }, { opacity: 1, transform: "translateY(0)" }], { duration: 240, easing: EASE_OUT });
      follow();
      return;
    } else {
  ...
  async function runTool(turn, name) {
    const tool = TOOLS[name];
    setStatus(tool.label);
    turn.insertAdjacentHTML("beforeend", toolMarkup({ name, args: tool.args }, false));
    const card = turn.lastElementChild;
    follow();
    await wait(reduceMotion() ? 150 : 500 + Math.random() * 500);
    const result = tool.run(p());
    card.querySelector(".c-tool__result").textContent = result;
    card.querySelector(".c-tool__status").innerHTML = icon("check");
  ...
  async function agentRun(text) {
    ...
    try {
      ...
      turn.insertAdjacentHTML("beforeend", thoughtMarkup(thought, seconds));
      ...
      await runTool(turn, "read_revenue");
      await runTool(turn, "read_users");
      // no question back: the answer covers the last 30 days, what the
      // badge compares against
      await runTool(turn, "list_insights");
      if (!root.isConnected) return;
  ...
  async function liveRun(text) {
    setBusy(projectId, "agent");
    form.dataset.busy = "1";
    showThinking();
    try {
      const res = await sendLive(projectId, text, p());
      ...
      // Only a fresh mission waits out the whole turn
      if (res?.ok && res.handled === "note") {
        setStatus("Suggestion added to the running work");
        return;
      }
      // the backend clears the busy flag on its own status events; we
      // only wait it out
      while (isBusy(projectId)) await wait(600);
    } catch (err) {
      console.error(err);
    } finally {
      setBusy(projectId, null);
  ...
  function send(text) {
    ...
    pushThread(projectId, { kind: "user", text, files: files.length ? files : undefined }, { own: true });
    ...
    runs = runs.then(() => (liveConnected() ? liveRun(text || `I attached ${files.join(", ")}`) : agentRun(text || `I attached ${files.join(", ")}`))).catch((err) => console.error(err));
  }
  ...
  mountSetups();
  restNow();
  // a reload mid-run: the thinking marker comes straight back up — unless
  // the chat is paused, which reads as idle everywhere
  if (!p().paused && (busyBy(projectId) === "agent" || isRunning(p()))) showThinking();
  ...
  const off = subscribe((topic, payload) => {
    ...
    if (topic === "thread" && payload.id === projectId && !payload.own) enqueue(payload.item);
      ...
      if (!p().paused) setStatus(payload.text || "Thinking");
    }
    ...
    else if (topic === "projects" && payload.changed === projectId) chat.querySelector("[data-chat-title]").textContent = p().name;
